    ...
    switch ($form_state->getValue('type')) {
      case 'basic':
        ...
        if (!trim($form_state->getValue('username'))) {
          $form_state->set('username', 'Username is required for Basic type');
        }

        if ($state_data = $this->stateManager->get('api_ai_webhook.auth', [])) {
          if (!isset($state_data['password']) && !trim($form_state->getValue('password'))) {
            $form_state->set('password', 'Password is required for Basic type');
          }
        }
        break;

      case 'headers':
        if (!trim($form_state->getValue('headers'))) {
          $form_state->set('headers', 'Headers key/value are required type');
        }
        break;
    }
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    switch ($type) {
      ...
      case 'headers':
        ...
        $headers = [];
        $line = strtok($form_state->getValue('headers'), "\r\n");
        while ($line) {
          $header = explode(':', $line);
        ...
        $config->set('auth.values.headers', array_keys($headers));
